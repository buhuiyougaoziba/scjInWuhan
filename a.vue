<template>
    <div class="recent-activity-main-container">
        <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
            <mt-loadmore ref="loadmore" :top-method="pullDownRefresh" :topPullText="topPullText"
                :topDropText="topDropText" :topLoadingText="topLoadingText">
                <div v-for="(item, index) in regionList" :key="index" @click="toDetailPage(item)">
                    <div class="recent-activity-view-container">
                        <div style="flex: 1">
                            <div class="recent-activity-title-container">
                                <div style="flex: 1; font: bold 16px PingFangSC">
                                    {{ item.activity_name }}
                                </div>
                                <a @click.stop="toDetail(item)" style="
                                        font: bold 16px PingFangSC;
                                        margin-right: 0;
                                    " :style="getJoinStyle(item.residents_id)">
                                    {{item.residents_id | joinState(residentInfo.residentsId)}}
                                </a>
                            </div>
                            <div class="recent-activity-info-container">
                                <div class="info-common-type">
                                    <img class="img" src="../img/i-time.png" alt="" />
                                    <span class="span">{{ item.hold_time | timeFormate }}</span>
                                </div>
                                <div class="info-common-type">
                                    <img class="img" src="../img/i-position.png" alt="" />
                                    <span class="span">{{ item.address }}</span>
                                </div>
                                <div class="info-common-type">
                                    <img class="img" src="../img/i-username.png" alt="" />
                                    <span class="span">{{ item.participateNum }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div v-if="(!regionList || regionList.length <= 0) && !isLoading">
            <img style="width: 100%; margin-top: 28px" src="../img/no_result.png" />
...
<script>
    import {
        bindNavbar
    } from "egova/js-interface";
    import {
        initBaseGData
    } from "../../hwPointManage/api/index";
    import {
        Search,
        InfiniteScroll,
        Loadmore
    ...
    export default {
        ...
        methods: {
            getJoinStyle(value) {
                ...
                if (value) {
                    value = value + "";
                    console.log("value : " + JSON.stringify(value) + ",," + this.residentInfo.residentsId)
                    let ids = value.split(",");
                    const content = ids.find(id => id == this.residentInfo.residentsId);
                    ...
                    if (data && data.success) {
                        ...
                        if (data.code == 501) {
                            ...
                            pubApi.getSmtUserInfo(this.smtUserId).then(data => {
                                ...
                                console.log("getSmtUserInfo -- " + JSON.stringify(
                                    data))
                                if (data && data.success) {
                                    ...
                                    pubApi.saveResidentInfo(resident).then(data => {
                                        if (data && data.success) {
                    ...
                    pubApi.getHWResidentBySmtUserId(this.smtUserId).then(data => {
                        ...
                        if (data && data.success) {
                            ...
                            if (data.code == 501) {
                                ...
                                pubApi.getSmtUserInfo(this.smtUserId).then(data => {
                                    // 获取到市民通user信息，再构建一个环卫居民，然后新增居民
                                    console.log("getSmtUserInfo -- " + JSON.stringify(data))
                                    if (data && data.success) {
                                        // let realName = data.data.userInfo.realName || "";
                                        let userName = data.data.userInfo.userName || "";
                                        let phone = data.data.userInfo.phone || "";
            ...
            getRegionList() {
                let that = this;
                Indicator.open();
                api.getActivityList(this.residentInfo.residentsId,
                        this.currentPage, this.numPerPage)
                    .then(data => {
                        Indicator.close();
                        that.isLoading = false;
                        if (data && data.success) {
                            let dataList = (data.data && data.data.resultList) || []
                            let pageInfo = (data.data && data.data.pageInfo) || null
                            if (that.isRefresh) {
            ...
            toDetail(item) {
                ...
                iMessageBox.confirm({
                    vm: this,
                    title: "报名确认",
                    message: "<div style='font: 14px PingFangSC;line-height: 20px'>" +
            ...
            joinActivity(activity) {
                api.residentsparticipate(this.residentInfo.residentsId, activity.activity_id).then(data => {
